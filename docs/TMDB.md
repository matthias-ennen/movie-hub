...
Der Zeitplan ist täglich um **03:17 Uhr in `Europe/Berlin`**. Da GitHub Cron-Ausdrücke in UTC auswertet, decken zwei UTC-Trigger die deutsche Sommer- und Winterzeit ab. Eine vorgeschaltete tägliche Sperre startet nur den ersten fälligen Datenlauf und überspringt den zweiten Trigger, sobald für denselben deutschen Kalendertag bereits ein erfolgreicher oder laufender Nachtlauf existiert. Ein fehlgeschlagener erster Versuch blockiert den Ersatztrigger nicht. Derselbe Workflow kann bei Bedarf auch manuell gestartet werden. Der Datenlauf protokolliert geplanten und tatsächlichen Start sowie die Verzögerung. Ein unabhängiger Wächter prüft nach dem vorgesehenen Start in zwei UTC-Fenstern, ob der Nachtlauf vorhanden und höchstens 60 Minuten verspätet ist, und markiert einen fehlenden, verspäteten oder fehlgeschlagenen Lauf sichtbar als Fehler. Schlägt ein Abruf, die Mindestprüfung oder der Build fehl, wird nichts veröffentlicht. Das bisherige funktionierende Katalog-Artefakt bleibt dann live.
...
Persistente Änderungs-IDs bleiben als Nachholschutz bis zu 30 Tage erhalten, werden aber nicht 30 Tage lang erneut geladen. Jeder Verbraucher vergleicht den Änderungszeitpunkt `lastSeen` mit `metadataUpdatedAt` beziehungsweise bei Staffelshards mit `generatedAt`. Eine mindestens gleich neue Kopie gilt als verarbeitet. Benötigt eine andere Quellmitgliedschaft noch denselben Stand, verteilt der zentrale Executor diese Kopie ohne zweiten TMDB-Detailabruf.

Der breite Suchindex ist ein dynamischer Anbieter-Snapshot und kein dauerhaft wachsendes Titelarchiv. Jeder Datenlauf vergleicht deshalb den vorherigen veröffentlichten Suchindex mit der neuen Generation. Für jede Kombination aus Anbieter, Medientyp und Angebotsart werden Seitenzahl, Rohresultate, akzeptierte Resultate, eindeutige Titel, Begrenzung am konfigurierten Seitenlimit und übersprungene Kandidaten protokolliert. Anfangs-, End- und Höchstwert der von TMDB gemeldeten Seitenzahl bleiben dabei getrennt sichtbar, weil sich die Trefferzahl während eines laufenden Scans ändern kann. Der vollständige Diagnosebericht enthält hinzugekommene und entfallene Titel sowie geänderte Anbieterangebote und wird 30 Tage als GitHub-Workflow-Artefakt aufbewahrt. Der kompakte Laufbericht zeigt die Summen und die Anzahl vollständig ausgeführter Scans. Ein sinkender Bestand gilt damit nicht automatisch als Fehler; entscheidend ist, ob der neue Snapshot vollständig erzeugt und seine Veränderung technisch nachvollziehbar ist.

Vor der Veröffentlichung gilt zusätzlich ein toleranter Plausibilitäts-Gate. Er verlangt den letzten Live-Suchindex als Vergleichsbasis, mindestens 155 vollständig ausgeführte Scans und eine Quote übersprungener TMDB-Kandidaten von höchstens 0,1 Prozent. Ein Rückgang des Gesamt-, Film- oder Serienbestands um mehr als 10 Prozent blockiert die Veröffentlichung. Die Schwellen liegen bewusst oberhalb der im kontrollierten Diagnoselauf gemessenen normalen Marktdynamik; ein blockierter Lauf lädt seinen bereits erzeugten Diagnosebericht trotzdem zur Ursachenanalyse hoch.
