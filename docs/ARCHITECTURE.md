...
Lokale Search-Detail-Caches sind an die Generation des veröffentlichten Detail-Manifests gebunden. Nach einem neuen Datenlauf werden ältere oder noch unversionierte Cacheeinträge deshalb nicht mehr als kanonisch verwendet; der passende Detail-Shard wird neu geladen. Beim Übergang führt die Oberfläche einen frischeren persönlichen Snapshot mit einem bereits vorhandenen, schwächeren Katalogeintrag zusammen und berechnet die sichtbare Artwork-Auswahl danach erneut. Damit können Poster und Hero-Backdrop sofort erscheinen, ohne einen titelbezogenen Reparaturpfad oder einen dauerhaften zweiten Metadatenbestand einzuführen.
